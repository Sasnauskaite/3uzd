...
void SeparateList1(T& S, G& Good, B& Bad, char choice)
{
	if(choice == 't')
	{
		...
		for(Studentas z : S)
		{
			if(z.get_avg()>=d)
			{
				student1.set_name(z.get_name());
				student1.set_lastname(z.get_lastname());
				student1.set_avg(z.get_avg());
				student1.set_med(z.get_med());
		...
		for(Studentas z : S)
		{
			if(z.get_avg()<d)
			{
				student2.set_name(z.get_name());
				student2.set_lastname(z.get_lastname());
				student2.set_avg(z.get_avg());
				student2.set_med(z.get_med());
	...
	if(choice == 't')
	{
		...
		while(i<studs)
		{
			if(S[i].get_avg()>=d)
			{
				...
				Good[gal].set_name(S[i].get_name());
				Good[gal].set_lastname(S[i].get_lastname());
				Good[gal].set_avg(S[i].get_avg());
				Good[gal].set_med(S[i].get_med());
				gal+=1;
			}
			else if(S[i].get_avg()<d)
			{
				Bad.push_back(Vargselis());
				Bad[varg].set_name(S[i].get_name());
				Bad[varg].set_lastname(S[i].get_lastname());
				Bad[varg].set_avg(S[i].get_avg());
				Bad[varg].set_med(S[i].get_med());
				varg+=1;
			}
			i++;
		}
...
void ReadToList(T& S)
{
	...
	try 
	{
		...
		for (int i = 0; !in.eof(); i++)
		{
			...
			in >> egz;
			student.set_avg(average());
			student.set_med(mediana());
			n = i;
			S.push_back(student);
		}
		n = n + 1;
...
void BubbleSort(T& S)
{
	for (int i = 0; i < S.size(); i++)
	{
		...
		{
			if (S[i].get_lastname() > S[j].get_lastname())
			{
				string a = S[i].get_lastname();
				S[i].set_lastname(S[j].get_lastname());
				S[j].set_lastname(a);
                string b = S[i].get_name();
				S[i].set_name(S[j].get_name());
				S[j].set_name(b);
                double c = S[i].get_avg();
				S[i].set_avg(S[j].get_avg());
				S[j].set_avg(c);
                double d = S[i].get_med();
				S[i].set_med(S[j].get_med());
				S[j].set_med(d);
			}
		}
	}
