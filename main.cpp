...
int nd[maxStudents][maxGrades]{}; //globalus kintamieji
float vid[maxStudents],  med[maxStudents];
int sk, egz, nr=0;
string name[maxStudents], lastname[maxStudents], line="-";
...
void mediana() //apskaiciuoja galutini bala su mediana
{
    ...
    if (sk % 2 != 0)
    {
        ...
        med[nr-1] = (0.4 * nd[nr][num]) + (0.6 * egz);
    }
    else if (sk % 2 == 0)
    {
        num = sk / static_cast<float>(2);
        number1 = num - 1;
        med[nr-1] = ((static_cast<float>(nd[nr][num]) + nd[nr][number1]) / 2);
